...
int main()
{
    ...
    cout << "\n" << "(If the student does not have the course, please enter" << " 'space' ) "<< "\n" ;
    for (int i = 0 ; i < ns ; i++)
    {
        ...
        //cin >> rp[i].num_st ;
        while ( (!(cin >> rp[i].num_st ) ) || ( numm(rp[i].num_st) != 9))
        {
        ...
        for (int j = 0 ; j < ( 2 * nc ) ; j=j+2)
        {
            ...
            //cin >> score[i][j] ;
            while ( (!(cin >> score[i][j] ) ) || ( score[i][j] > 20 ) || (score[i][j] < 0) )
            {
                cout <<"Please enter  "<<"\033[1m9 Number\033[0m\n" <<"lll"<<"\n" ;
                cin.clear() ;
                cin.ignore(numeric_limits<streamsize>::max() , '\n') ;
            }
    ...
    //cout << "If you need to see specific Student , Enter 1 :" << "\n" ;
